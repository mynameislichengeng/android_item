...
public abstract class HttpHeaderFactory {
    ...
    public String getWeiboAuthHeader(String method, String url, BaseParameters params){
    	try {
    		final long timestamp = System.currentTimeMillis() / 1000;
            final long nonce = timestamp + (new Random()).nextInt();
            // step 2: authParams�������ô���1.���ܴ�һ���� 2.������Authorizationͷ��
            BaseParameters authParams = this.generateAuthParameters(nonce, timestamp);
            // ������ڼ���signature�ģ�����
            BaseParameters signatureParams = this.generateSignatureParameters(authParams, params, url);
    ...
    private String generateAuthSignature(final String method, BaseParameters signatureParams,
            final String url) {
    ...
    private BaseParameters generateSignatureParameters(BaseParameters authParams,
    		BaseParameters params, String url){
    	BaseParameters signatureParams = new BaseParameters();
        signatureParams.addAll(authParams);
        signatureParams.addAll(params);
        this.parseUrlParameters(url, signatureParams);
        BaseParameters lsp = generateSignatureList(signatureParams);
        return lsp;
    }

    private BaseParameters generateAuthParameters(long nonce, long timestamp){
    	BaseParameters authParams = new BaseParameters();
    ...
    public abstract BaseParameters generateSignatureList(BaseParameters bundle);

    // add additional parameters to des key-value pairs,support to expanding
    // params
    public abstract void addAdditionalParams(BaseParameters des, BaseParameters src);

    // ����url�в����,�洢��signatureBaseParams
    public void parseUrlParameters(String url, BaseParameters signatureBaseParams){
    ...
    public static String encodeParameters(BaseParameters postParams, String splitter, boolean quot) {
