...
public class GroupContactListAdapter extends BaseAdapter implements SectionIndexer {
	...
	public GroupContactListAdapter(Context context, List<PersonBean> personList) {
...
//		 holder.contactFirstLetterText.setText(bean.getAvatar());
		...
		imageLoader.displayImage("file://" + Constants.SAVE_IMAGE_PATH + "HeadAvatar/" + bean.getAvatar().substring(bean.getAvatar().lastIndexOf("/") + 1, bean.getAvatar().length()), holder.avatarImage, options);
		return convertView;
	}
