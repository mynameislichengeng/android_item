...
public class ServerAdaptor {
	 ...
	 * 执行动作
	 */
	public void doAction1(String url,int viewInstance, String actionName, Object parameters, ServiceSyncListener listener) throws HaoqeeException {
		// 根据viewInstance创建（恢复）View上下文环境;
		// 根据actionName反射ActionController的doAction方法，并调用;
		// 返回结果;
		HashMap<String, Object> body = new HashMap<String, Object>();
		...
		sendAsyncJsonMessage1(url,"doAction", body, listener);
	}
...
//	}
	 ...
	 *            回调函数
	 */
	private void sendAsyncJsonMessage1(String url,String methodName, Map<String, Object> parameters, ServiceSyncListener listener) {
		// JSON格式转换
		JSONObject requestJsonObject = new JSONObject(parameters);
		...
		asyncJsonTask.listener = listener;
		asyncJsonTask.context = context;
		asyncJsonTask.setPostType(AsyncJsonTask.POST_HTTP);
		asyncJsonTask.setGetUrl(url);
		asyncJsonTask.execute(requestJsonObject);
...
//	}
	 ...
	 * @param listener
	 */
	public void doGetAction(String url, ServiceSyncListener listener) throws HaoqeeException {
		sendGetAsyncJsonMessage(url, listener);
	}

	private void sendGetAsyncJsonMessage(String url, ServiceSyncListener listener) {
		AsyncJsonTask asyncRequestTask = AsyncJsonTask.instance();
		asyncRequestTask.setPostType(AsyncJsonTask.GET_HTTP);
		asyncRequestTask.listener = listener;
		asyncRequestTask.context = context;
		asyncRequestTask.setGetUrl(url);
