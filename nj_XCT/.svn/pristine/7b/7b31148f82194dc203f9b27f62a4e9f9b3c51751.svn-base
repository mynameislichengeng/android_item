...
public class DESEncrypter {
	public static String strDefaultKey="jsdy2012";
	...
	public byte[] getEncryptKey() {
		System.out.println(strDefaultKey);
		return strDefaultKey.getBytes();
	}

	/*
	 * 加密
	 */
	public byte[] getEncryptData(String inputStr)
			throws UnsupportedEncodingException {
		int encryptDataLen;
		DESEncrypter _Des = new DESEncrypter();
		byte[] byte2 = new byte[3000];
		byte[] byte1 = inputStr.getBytes("UTF-8");
		byte[] byteMiW = new byte[3000];
		encryptDataLen = _Des.ENCRYPT(_Des.getEncryptKey(), byte1, byte2,
				byte1.length);
		digestHexStr = "";
		for (int i = 0; i < encryptDataLen; i++) {

			digestHexStr += byteHEX(byte2[i]);
		}
		System.out.println("密文:[" + digestHexStr + "]");
		byteMiW = _Des.hexStr2ByteArr(digestHexStr);
		return byteMiW;
	}
	 ...
	 * 解密
	 */
	public String getDecryptData(byte[] byteMiW)
			throws UnsupportedEncodingException {
		DESEncrypter _Des = new DESEncrypter();
		byte[] byteMingW = new byte[3000]; // 明文
		_Des.DECRYPT(_Des.getEncryptKey(), byteMingW, byteMiW, byteMiW.length);
		return new String(new String(byteMingW, "UTF-8"));
	}

//	public static void main(String args[]) throws UnsupportedEncodingException {
//		DESEncrypter _Des = new DESEncrypter();
//		_Des.strDefaultKey="09876543";
//
//		byte[] MiW = new byte[3000];
//		MiW = _Des.getEncryptData("{username:'admin',password:'123456'}");
//		System.out.println("明文:["
//				+ _Des.getDecryptData(MiW).replaceAll("\0", "") + "]");
//		System.out.println("明文:" + Tools.base64Encoder(MiW));
//
//		System.out
//				.println("base64明文:["
//						+ _Des
//								.getDecryptData(
//										Tools
//												.base64DecoderToBytes("74bc2RdWHGyCbQaSmXJhkG13vectDO3PfmFlQm9ROCQjOveK1ZQ3/Q=="))
//								.replaceAll("\0", "") + "]");
//		System.out
//				.println("base64明文:["
//						+ _Des
//								.getDecryptData(
//										Tools
//												.base64DecoderToBytes("TSf0Ymt0EvMQI0YmKbzznMWyn3dT+HUY1tFaebfj2W63tiylYZqoaw=="))
