...
import com.xguanjia.hx.contact.service.PersonSelfService;

public class ContactOrganizeShareAdapter extends BaseAdapter {

	private List<PersonBean> _personList;
	private PersonSelfService personSelfService;
	...
	public ContactOrganizeShareAdapter(List<PersonBean> personList, Context context) {
		this._personList = personList;
		this._context = context;
		options = new DisplayImageOptions.Builder().showStubImage(R.drawable.default_portrait).showImageForEmptyUri(R.drawable.default_avatar).cacheInMemory().cacheOnDisc().displayer(
				new RoundedBitmapDisplayer(10)).imageScaleType(ImageScaleType.IN_SAMPLE_INT).build();
		imageLoader.init(ImageLoaderConfiguration.createDefault(context));
		personSelfService = new PersonSelfService(context);
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (bean.getGroupId() != null && !"".equals(bean.getGroupId())) {
			...
			holder.contactFirstLetterText.setImageResource(R.drawable.default_organization);
		} else {
