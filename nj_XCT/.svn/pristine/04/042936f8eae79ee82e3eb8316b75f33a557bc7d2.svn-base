...
public class RefreshListViewWork extends ListView implements OnScrollListener,
		OnClickListener {
	...
	public RefreshListViewWork(Context context) {
		super(context);
		init(context);
	}

	public RefreshListViewWork(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}

	public RefreshListViewWork(Context context, AttributeSet attrs, int defStyle) {
...
//		mLastUpdateTextView.setText("最近更新:" + new Date().toLocaleString());
	...
	{
		mViewState = IListViewState.LVS_NONE;
	}
	...
	// 初始化footview试图
	public void initLoadMoreView(Context context) {
...
//			mRefreshTextview.setText("松开获取更多");
	...
	// 无数据
	public void onLoadMoreDisMiss()
	{
		updateLoadMoreViewState(ILoadMoreViewState.LMVS_DISMISS);
	}
	...
	private void updateLoadMoreViewState(int state) {
		...
		case ILoadMoreViewState.LMVS_OVER:
			mLoadingView.setVisibility(View.GONE);
			mLoadMoreTextView.setVisibility(View.VISIBLE);
			mLoadMoreTextView.setText("暂无新数据");
			break;
		case ILoadMoreViewState.LMVS_DISMISS:
			mLoadingView.setVisibility(View.GONE);
			mLoadMoreTextView.setVisibility(View.GONE);
			break;
		default:
