...
public class PersonService {
	...
	public String queryPerson(String loginId) {
		...
		String sql = "select * from tb_person where pe_user_id = ?";
	 ...
	 * 根据人员ID查询人员信息
	 */
	public List<PersonBean> queryDefaultPerson(String loginId) {
		database = dbOpenHelper.getReadableDatabase();
		String sql = "select * from tb_person where pe_user_id = ?";
		Cursor cursor = database.rawQuery(sql, new String[] { loginId });
		List<PersonBean> list = getPersonFromCursor(cursor);
	 ...
	 * 根据loginName查询登录人员姓名
	 */
	public String queryPersonName(String loginName) {
		database = dbOpenHelper.getReadableDatabase();
		String sql = "select * from tb_person where pe_login_name = ?";
		Cursor cursor = database.rawQuery(sql, new String[] { loginName });
		List<PersonBean> list = getPersonFromCursor(cursor);
		if (list.size() > 0) {
			cursor.close();
			return list.get(0).getName();
	 ...
	 * @param personType
	 */
	public void savePerson(List<PersonBean> list, int personType,
			boolean isDeleateAllPerson) {
		database = dbOpenHelper.getWritableDatabase();
		...
		for (int i = 0; i < j; i++) {
			...
			sortKeyName = PingYinUtil.changeToPingYin(bean.getName(), false)
					+ bean.getName() + bean.getMobile();
			if (!"".equals(sortKeyName)) {
				sortIndex = sortKeyName.substring(0, 1);
			}
			String sql = "insert into tb_person(primary_id,pe_parent_id,pe_group_id,pe_user_id,pe_text,pe_name,pe_nick_name,pe_birthday,pe_avatar,pe_mobile,pe_home_phone,pe_work_phone,pe_work_fax,pe_skype,pe_msn,pe_qq,pe_unit_name,pe_email,pe_work_email,pe_mobile_email,pe_unit_address,pe_home_address,pe_remark,pe_contacts_name,pe_contacts_id,pe_person_type,pe_sortkey,pe_sortIndex,pe_login_id,pe_post,pe_login_name,pe_uid) values("
					+ "'"
					+ key
					+ "','"
					+ bean.getParentId()
	...
	public List<PersonBean> queryChildOrg(String groupId) {
		...
		database = dbOpenHelper.getWritableDatabase();
		String sql = "select * from tb_person where pe_parent_id = ?";
	 ...
	 * @return
	 */
	public List<PersonBean> queryPersonByType(int personType, String groupId,
			String sortKey, int queryMode, boolean isFirstSerch) {

		List<PersonBean> list = new ArrayList<PersonBean>();
		// database = getDatabase();
		database = dbOpenHelper.getWritableDatabase();
		StringBuilder sql = new StringBuilder(
				"select * from tb_person where pe_login_id = ? and ");
		Cursor cursor = null;
		String sql2 = "select * from tb_person where pe_login_id = ? and pe_group_id = 2";
		boolean flag = true;
		if (isFirstSerch) {
			...
			if (cursor.moveToFirst()) {
				...
				sql2 = "select * from tb_person where pe_login_id = ? and pe_group_id = 1";
				cursor = database.rawQuery(sql2,
				...
				// 直接查询出内部联系人
				sql2 = "select * from tb_person where pe_login_id = ? and pe_parent_id = 1";
				cursor = database.rawQuery(sql2,
						new String[] { Constants.userId });
	...
	public List<PersonBean> getPersonFromCursor(Cursor cursor) {
		...
		while (cursor.moveToNext()) {
			...
			bean.setUid(cursor.getString(cursor.getColumnIndex("pe_uid")));
			list.add(bean);
		}
		return list;
	}
	...
	public void deletePerson() {
		...
		if (null != database) {
			try {
				database.execSQL("delete from tb_person");
				database.setTransactionSuccessful();
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
		...
		if (null != database) {
			try {
				for (int i = 0; i < personIds.size(); i++) {
					database.execSQL(
							"delete from tb_person where primary_id = ?",
							new Object[] { personIds.get(i) });
				}
				database.setTransactionSuccessful();
			} catch (Exception e) {
				e.printStackTrace();
	...
	public List<PersonBean> queryInternalPerson() {
		String sql = "select * from tb_person where pe_login_id = ? and pe_user_id!= ? and pe_user_id !=''";
		SQLiteDatabase database = dbOpenHelper.getReadableDatabase();
		Cursor cursor = database.rawQuery(sql, new String[] { Constants.userId,
				Constants.userId });
		List<PersonBean> list = getPersonFromCursor(cursor);
		return list;
	}
	...
	// boolean flag = true;
	// String sql =
	// "select pe_parent_id,pe_group_id,pe_text,pe_name,pe_post,pe_qq from tb_person where pe_mobile = ?";
	// SQLiteDatabase database = dbOpenHelper.getReadableDatabase();
	...
	// bean.setQq(cursor.getString(cursor.getColumnIndex("pe_qq")));
	// sql =
	// "select pe_name,pe_parent_id,pe_group_id from tb_person where pe_group_id = ?";
	// while (flag) {
	...
	public List<PersonBean> queryInterDept() {
		...
		Cursor cursor = null;
		database = dbOpenHelper.getReadableDatabase();
		String sql2 = "select * from tb_person where pe_login_id = ? and pe_parent_id = 1";
		cursor = database.rawQuery(sql2, new String[] { Constants.userId });
		list = getPersonFromCursor(cursor);
		if (null != database) {
			if (null != cursor) {
	...
	public void attentionPerson(String isAttention, String primaryId) {
		...
		if (null != database) {
			try {
				...
				database.execSQL(sql.toString(), new String[] { isAttention,
						primaryId });
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
	...
	public List<PersonBean> queryAttactionPersonList() {
		...
		database = dbOpenHelper.getWritableDatabase();
		StringBuilder sql = new StringBuilder(
				"select * from tb_person where pe_login_id = ? and pe_skype = 1");
	...
	public List<String> queryUserInfoByPhoneNum(String phoneNum) {
		...
		boolean flag = true;
		// String sql =
		// "select * from tb_person where (pe_mobile = ? or pe_work_phone = ?) and pe_parent_id = (select min(pe_parent_id) from tb_person where (pe_mobile = ? or pe_work_phone = ?))";
		// String sql =
		// "select * from tb_person where pe_mobile = ? or pe_work_phone = ?";
		String sql = "select * from tb_person where pe_mobile like ?";
		Cursor cursor = null;
		try {
			SQLiteDatabase database = dbOpenHelper.getReadableDatabase();
			...
			if (cursor.moveToFirst()) {
				...
				sql = "select * from tb_person where pe_group_id = ?";
				// while (flag) {
				cursor = database.rawQuery(sql,
