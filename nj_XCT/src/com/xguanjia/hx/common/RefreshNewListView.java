...
public class RefreshNewListView extends ListView implements OnScrollListener,
		OnClickListener {
	...
	public RefreshNewListView(Context context) {
		super(context);
		init(context);
	}

	public RefreshNewListView(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context);
	}
		...
		if (mOnRefreshListener != null) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				if (position != INVALID_POSITION) {
					HaoXinMsgBean data = (HaoXinMsgBean) getItemAtPosition(position);
...
//			mRefreshTextview.setText("下拉可以刷新");
		...
		case IListViewState.LVS_RELEASE_REFRESH: {
			...
			mHeadProgressBar.setVisibility(View.GONE);
			mArrowImageView.setVisibility(View.VISIBLE);
			mRefreshTextview.setText("释放立即更新");
			mArrowImageView.clearAnimation();
