...
public class PersonAllService {
	...
	public PersonAllService(Context context) {
	...
	public String queryPerson(String loginId) {
		...
		String sql = "select * from tb_allperson where pe_user_id = ?";
	...
	public void savePerson(List<PersonBean> list, int personType, boolean isDeleateAllPerson) {
		...
		if (isDeleateAllPerson) {
			database.execSQL("delete from tb_allperson", new Object[]{});
		}
		...
		for (int i = 0; i < j; i++) {
			...
			String sql = "insert into tb_allperson(primary_id,pe_parent_id,pe_group_id,pe_user_id,pe_text,pe_name,pe_nick_name,pe_birthday,pe_avatar,pe_mobile,pe_home_phone,pe_work_phone,pe_work_fax,pe_skype,pe_msn,pe_qq,pe_unit_name,pe_email,pe_work_email,pe_mobile_email,pe_unit_address,pe_home_address,pe_remark,pe_contacts_name,pe_contacts_id,pe_person_type,pe_sortkey,pe_sortIndex,pe_login_id,pe_post) values("
					+ "'"
					+ key
					+ "','"
					+ bean.getParentId()
					...
					+ "','"
					+ personType
					+ "','" + sortKeyName + "','" + sortIndex + "','" + Constants.userId + "','" + bean.getPost() + "')";
			sqlList.add(sql);
		}
	...
	public List<PersonBean> queryChildOrg(String groupId) {
		// database = getDatabase();
		database = dbOpenHelper.getWritableDatabase();
		String sql = "select * from tb_allperson where pe_parent_id = ?";
	...
	public List<PersonBean> queryPersonByType(int personType, String groupId, String sortKey, int queryMode, boolean isFirstSerch) {
		...
		// database = getDatabase();
		database = dbOpenHelper.getWritableDatabase();
		StringBuilder sql = new StringBuilder("select * from tb_allperson where pe_login_id = ? and ");
		Cursor cursor = null;
		String sql2 = "select * from tb_allperson where pe_login_id = ? and pe_group_id = 2";
		boolean flag = true;
		if (isFirstSerch) {
			...
			if (cursor.moveToFirst()) {
				...
				sql2 = "select * from tb_allperson where pe_login_id = ? and pe_group_id = 1";
				cursor = database.rawQuery(sql2, new String[]{Constants.userId});
				...
				// 直接查询出内部联系人
				sql2 = "select * from tb_allperson where pe_login_id = ? and pe_parent_id = 1";
				cursor = database.rawQuery(sql2, new String[]{Constants.userId});
	...
	public void deletePerson() {
		...
		if (null != database) {
			try {
				database.execSQL("delete from tb_allperson");
				database.setTransactionSuccessful();
			} catch (Exception e) {
				e.printStackTrace();
			} finally {
		...
		if (null != database) {
			try {
				for (int i = 0; i < personIds.size(); i++) {
					database.execSQL("delete from tb_allperson where primary_id = ?", new Object[]{personIds.get(i)});
				}
				database.setTransactionSuccessful();
			} catch (Exception e) {
				e.printStackTrace();
	...
	public List<PersonBean> queryInternalPerson() {
		String sql = "select * from tb_allperson where pe_login_id = ? and pe_user_id!= ? and pe_user_id !=''";
		SQLiteDatabase database = dbOpenHelper.getReadableDatabase();
...
//		boolean flag = true;
//		String sql = "select pe_parent_id,pe_group_id,pe_text,pe_name,pe_post,pe_qq from tb_allperson where pe_mobile = ?";
//		SQLiteDatabase database = dbOpenHelper.getReadableDatabase();
...
//			bean.setPost(cursor.getString(cursor.getColumnIndex("pe_post")));
//			bean.setQq(cursor.getString(cursor.getColumnIndex("pe_qq")));
//			sql = "select pe_name,pe_parent_id,pe_group_id from tb_allperson where pe_group_id = ?";
//			while (flag) {
...
//	}
	...
	public List<PersonBean> queryInterDept() {
		...
		String sql2 = "select * from tb_allperson where pe_login_id = ? and pe_parent_id = 1";
		cursor = database.rawQuery(sql2, new String[]{Constants.userId});
		list = getPersonFromCursor(cursor);
		if (null != database) {
			if (null != cursor) {
	...
	public void attentionPerson(String isAttention, String primaryId) {
		...
		if (null != database) {
			try {
				StringBuilder sql = new StringBuilder("update tb_allperson set pe_skype = ? where primary_id = ?");
	...
	public List<PersonBean> queryAttactionPersonList() {
		List<PersonBean> data = new ArrayList<PersonBean>();
		database = dbOpenHelper.getWritableDatabase();
		StringBuilder sql = new StringBuilder("select * from tb_allperson where pe_login_id = ? and pe_skype = 1");
...
//		}
		...
		List<String> departmentList = new ArrayList<String>();
		boolean flag = true;
//		String sql = "select * from tb_allperson where (pe_mobile = ? or pe_work_phone = ?) and pe_parent_id = (select min(pe_parent_id) from tb_allperson where (pe_mobile = ? or pe_work_phone = ?))";
//		String sql = "select * from tb_allperson where pe_mobile = ? or pe_work_phone = ?";
		String sql = "select * from tb_allperson where pe_mobile like ?";
		Cursor cursor = null;
		try {
			SQLiteDatabase database = dbOpenHelper.getReadableDatabase();
			...
			if (cursor.moveToFirst()) {
				...
				bean.setPost(cursor.getString(cursor.getColumnIndex("pe_post")));
				bean.setAvatar(cursor.getString(cursor.getColumnIndex("pe_avatar")));
				sql = "select * from tb_allperson where pe_group_id = ?";
//				while (flag) {
