...
public class AudioPlay {
	...
	private void init(TCMessage messageInfo){
		if(USE_SYSTEM_MEDIA){
	 ...
	 * 修改时间:<br />
	 */
	public void play(TCMessage messageInfo, int type){
		play(messageInfo, type, false);
	}
	
	public void play(TCMessage messageInfo, int type, boolean hasCallBack){
		if(currUrl.equals(messageInfo.getVoiceMessageBody().getVoiceUrl()) && state){
			stop(hasCallBack);
			return;
		} else {
			stop(hasCallBack);
		}
		mMessageTag = messageInfo.getMessageTag();
		init(messageInfo);
		
		currUrl = messageInfo.getVoiceMessageBody().getVoiceUrl();
		String fileName = null;
		File file = null;
		if(currUrl.startsWith("http://")){
			fileName = FeatureFunction.generator(messageInfo.getVoiceMessageBody().getVoiceUrl());
